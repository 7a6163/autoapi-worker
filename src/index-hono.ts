...
app.post('/run-read', async (c) => {
  try {
    ...
    return c.json({ success: true, message: 'Read APIs executed successfully' });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return c.json({ success: false, error: errorMessage }, 500);
  }
...
app.post('/run-write', async (c) => {
  try {
    ...
    return c.json({ success: true, message: 'Write APIs executed successfully' });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return c.json({ success: false, error: errorMessage }, 500);
  }
...
async function scheduled(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    await sendTelegramMessage(env, `❌ AutoApi Worker 執行失敗: ${errorMessage}`);
  }
}

export async function sendTelegramMessage(env: WorkerEnv, message: string): Promise<void> {
